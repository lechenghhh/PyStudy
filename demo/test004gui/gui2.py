...
def main():
    ...
    text = edit(root)  #
    l = tk.Label(root, text='My name is Bin_LmmH_C', fg='white', bg='#0288D1', width=30)
...
def edit(root):
    edit = tk.Text(root, fg='white', bg='#0288D1', font='微软雅黑', width=30, height=10, )
    edit.grid(sticky=tk.N + tk.E + tk.W)

    # button 传递参数使用lambda函数
    # delete all the value in the text editor
    clear1 = tk.Button(root, text='Clear', width=27, bg='#00BCD4', font='微软雅黑',
                       command=lambda: edit.delete(1.0, tk.END))
    clear1.grid()

    result = tk.Text(root, fg='white', bg='#0288D1', font='微软雅黑', width=30, height=10, )
    result.grid(sticky=tk.N + tk.E + tk.W)

    # button 传递参数使用lambda函数
    # delete all the value in the text editor
    clear2 = tk.Button(root, text='Clear', width=27, bg='#00BCD4', font='微软雅黑',
...
def button(root, text):
    clu = 0
    b64en = tk.Button(root, text='Base64 Encode', fg='white', bg='#03A9F4', command=lambda: b64encode(text))
    b64de = tk.Button(root, text='Base64 Decode', fg='white', bg='#03A9F4', command=lambda: b64decode(text))
    b32en = tk.Button(root, text='Base32 Encode', fg='white', bg='#03A9F4', command=lambda: b32encode(text))
    b32de = tk.Button(root, text='Base32 Decode', fg='white', bg='#03A9F4', command=lambda: b32encode(text))
    md5do = tk.Button(root, text='-Md5  Creator-', fg='white', bg='#03A9F4', command=lambda: md5create(text))
